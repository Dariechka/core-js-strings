...
function removeLastOccurrences(str, value) {
  ...
  return str.slice(0, endIndex) + str.slice(endIndex + value.length);
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((result, item) => (arr.includes(item) ? result + 1 : result), 0);
}
  ...
  const strNew = str
    ...
    .flatMap((item) => {
      return item.includes(',') || item.includes('!') || item.includes('?')
        ? item.slice(0, item.length - 1).toLowerCase()
        : item.toLowerCase();
    })
    .join('');
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce(
      (acc, item) => (item.length > acc.length ? acc.replace(acc, item) : acc),
      ''
    );
}
...
function extractNameFromTemplate(value) {
  ...
  arr.splice(0, 1);
  const valueNew = arr.join(' ');
  return valueNew.slice(0, valueNew.length - 1);
}
